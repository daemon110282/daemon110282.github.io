...
- определяют, какие __функции и возможности__ должен предоставлять продукт для удовлетворения [бизнес требований](analytica.bt.md)
- описывают __поведение системы__ при взаимодействии с пользователями и другими системами

### Use Case (UC) Сценарии использования

Use Case (UC) - Сценарии использования - Варианты использования (ВИ):
...
- объединяет __функциональные требования__ по контексту для достижения __цели пользователя__, т.е. один UC может содержать __более одного ФТ на множество ИС__
  ...
  - Благодаря отсутствию привязки к элементам пользовательского интерфейса, UC становятся повторно используемыми требованиями, которые остаются актуальными при изменении платформы или других особенностей реализации
- Например:
...
- если ваш UC решает проблемы разработки ПО, то есть создаёт общее понимание требований у заказчика и всей проектной команды, то вы написали хороший UC
